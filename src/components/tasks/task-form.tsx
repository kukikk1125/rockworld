...
export function TaskForm({ initialTask }: { initialTask?: Task }) {
  ...
  const defaultPresetIndex = initialTask
    ? petPresets.findIndex((preset) => preset.petName === initialTask.petName)
    : 0;

  const [selectedPresetIndex, setSelectedPresetIndex] = useState(
    defaultPresetIndex >= 0 ? String(defaultPresetIndex) : "0",
  );
  const selectedPreset = petPresets[Number(selectedPresetIndex)];
  const [form, setForm] = useState<Task>(
    initialTask ?? createTaskFromPreset(selectedPreset),
  );
    ...
    <div className="grid gap-5">
      <Card className="bg-white/88 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="刷取模式">
            <Select
              ...
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  mode: e.target.value as TaskMode,
                }))
              }
            >
          ...
          <Field label="宠物类型">
            <Select
              ...
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  petType: e.target.value as Task["petType"],
                }))
              }
            >
          ...
          <Field label="所属系别/家族">
            <Input
              value={form.familyOrType}
              onChange={(e) =>
                setForm((prev) => ({ ...prev, familyOrType: e.target.value }))
              }
            />
          </Field>
          <Field label="图片路径">
          ...
          <Field label="A 系搭配">
            <Input
              value={form.aPet ?? ""}
              onChange={(e) => setForm((prev) => ({ ...prev, aPet: e.target.value }))}
            />
          </Field>
          <Field label="B 系搭配">
            <Input
          ...
          <Field label="刷取点位">
            <Input
              value={form.spawnLocation ?? ""}
              onChange={(e) =>
                setForm((prev) => ({ ...prev, spawnLocation: e.target.value }))
              }
            />
          </Field>
          <Field label="精灵果实">
            <Input
              value={form.fruitInfo ?? ""}
              onChange={(e) => setForm((prev) => ({ ...prev, fruitInfo: e.target.value }))}
            />
          </Field>
        </div>

        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Button size="lg" className="h-12 flex-1 rounded-2xl" onClick={handleSave}>
            保存修改
          </Button>
          <Button
            variant="outline"
            ...
            onClick={() => router.push(`/tasks/${form.id}`)}
          >
            返回任务页
          </Button>
        </div>
